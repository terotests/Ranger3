...
class tester  {
  ...
  walkNode (ctx) {
    ...
    if( ctx.activeNode instanceof RExpression ) /* union case */ {
      ...
      console.log("--- found expression node, but I am going to do nothing to it ----");
      return ctx;
    };
      ...
      if ( (typeof(myVar) !== "undefined" && myVar != null )  ) {
        ...
        if ( ((typeof(ctx.outerBlock) !== "undefined" && ctx.outerBlock != null ) ) && (typeof(defined_here) === "undefined") ) {
          ...
          if ( (typeof(outerVar) !== "undefined" && outerVar != null )  ) {
            console.log("^^ it was defined also in outer scope...");
            console.log("** captured " + b_3.name);
  ...
  run () {
    ...
    operatorsOf_7.forEach_19(ctx.variables, ((item, index) => { 
      console.log((((((("variable : " + index) + " type --> ") + item.typename) + " write cnt ") + item.write_cnt) + " read cnt ") + item.read_cnt);
    }));
    let walkFn = ((node) => { 
    });
    walkFn = ((node) => { 
      if( node instanceof RTextNode ) /* union case */ {
        var txt = node;
        console.log("txt = " + txt.text);
      };
      if( node instanceof RBlockNode ) /* union case */ {
        var b = node;
        console.log("");
        console.log("block with child count = " + operatorsOfVector.size_20(b.children));
        if ( (typeof(b.startCtx.outerBlock) !== "undefined" && b.startCtx.outerBlock != null )  ) {
          console.log(" ^^ has outerBlock ");
          const ob = b.startCtx.outerBlock;
          if ( (typeof(ob.outerBlock) !== "undefined" && ob.outerBlock != null )  ) {
            console.log(" ^^ and outer has also outer ! ");
          }
        }
        console.log("variables at the start of the block:");
        operatorsOf_7.forEach_19(b.startCtx.variables, ((item, index) => { 
          console.log(((" ** variable : " + index) + " type --> ") + item.typename);
          if ( (typeof(item.value) !== "undefined" && item.value != null )  ) {
            ...
            if( Number.isInteger ? Number.isInteger(v) : (function(v) { return typeof v === 'number' &&  isFinite(v) && Math.floor(v) === v; })(v) ) /* union case for int */ {
              var x = v;
              console.log("^ int value " + x);
            };
            if( typeof(v) === 'string' ) /* union case for string */ {
              var x_1 = v;
              console.log("^ string value " + x_1);
            };
          }
        }));
        console.log("variables at the end of the block:");
        operatorsOf_7.forEach_19(b.endCtx.variables, ((item, index) => { 
          console.log(((" ** variable : " + index) + " type --> ") + item.typename);
          if ( (typeof(item.value) !== "undefined" && item.value != null )  ) {
            const v_1 = item.value;
            if( v_1 instanceof RType_Literal ) /* union case */ {
              var x_2 = v_1;
              console.log((("^ int value " + x_2.int_value) + " str val ") + x_2.string_value);
            };
          }
        }));
        console.log("define cnt : " + (operatorsOf_7.keys_21(b.endCtx.defined_vars).length));
        console.log("capture cnt : " + (operatorsOf_7.keys_21(b.endCtx.captured_vars).length));
        console.log("");
        operatorsOfVector.forEach_2(b.children, ((item) => { 
          ...
          if( item instanceof RTextNode ) /* union case */ {
            var txt_1 = item;
            console.log("item.txt = " + txt_1.text);
          };
          if( item instanceof RBlockNode ) /* union case */ {
            var n = item;
            console.log("another block with child cnt = " + operatorsOfVector.size_20(n.children));
            walkFn(item);
          };
        }));
      };
    ...
    console.log("Error cnt : " + operatorsOfVector.size_22(ctx.errors));
    operatorsOfVector.forEach_23(ctx.errors, ((item) => { 
      if( item instanceof RError ) /* union case */ {
        var e_1 = item;
        console.log(e_1.text);
      };
    }));
    operatorsOf_4.forEach_26(operatorsOf_7.keys_25(ctx.defined_types), ((item, index) => { 
      console.log(" - " + item);
      const t = operatorsOf_7.get_13(ctx.defined_types, item);
      if( t instanceof RType_Class ) /* union case */ {
        var cl = t;
        operatorsOf_4.forEach_26(operatorsOf_7.keys_27(cl.variables), ((item, index) => { 
...
operatorsOfVector.size_20 = function(__self) {
  return (__self).count();
};
operatorsOfVector.size_22 = function(__self) {
  return (__self).count();
};
operatorsOfVector.forEach_23 = function(__self, cb) {
  const cnt_1 = (__self).count();
  let i_6 = 0;
  while (i_6 < cnt_1) {
    const item_1 = operatorsOfVector.itemAt_24(__self, i_6);
    cb(item_1);
    i_6 = i_6 + 1;
  };
};
operatorsOfVector.itemAt_24 = function(__self, idx) {
...
operatorsOf_4.forEach_26 = function(__self, cb) {
...
operatorsOf_7.forEach_19 = function(__self, cb) {
...
operatorsOf_7.keys_21 = function(__self) {
  return Object.keys(__self.elements);
};
operatorsOf_7.keys_25 = function(__self) {
  return Object.keys(__self.elements);
};
operatorsOf_7.keys_27 = function(__self) {
  return Object.keys(__self.elements);
};
